import re, ast, sys, traceback, json

def main():
    AST = astParser("example2.py",7)
    AST.astErrorCheck() # SyntaxError만 걸러줌
    # Name Error 내재한 채로 tree 형성
    AST.astDump(False, False)
    code = AST.astPreproc()
    AST.codeParser(code)
    AST.pathTracker(True)
    AST.astDump(True, True) # compile 결과를 낼 때, NameError가 Detection
    ...
    print("JSON")
    jsons = AST.ast2json(line_struct)
    print(len(AST.path), len(line_struct))
    path2json = []
    print(AST.json_dict)
    for path in AST.path:
        AST.json_dict[int(path)]["lineno"] = int(path)
        path2json.append(AST.json_dict[int(path)])
...
class astParser():
    ...
    def __init__(self, codes, depth):
        with open(codes, "r", encoding = "utf-8") as source:
            lines = source.read()
            lines = re.sub(r'\'\'\'[\w\W]+\'\'\'','',lines) # v2
                ...
                if 'SyntaxError: invalid syntax' in ERR[-2]:
                    ...
                    if paren:
                        ...
                        if len(ERR[-3]) == len(ERR[-4]):
                            ...
                            errline = int(ERR[-5].split()[-1])-1
                            print("비교 연산자를 주의하세요.")
                            print("Error line [{0}]:{1}\nChange Direction : {2}".format(errline+1,buffer[errline],buffer[errline].replace(ERR[-4][ERR[-3].index("^")], "==")))
                    ...
                    print("Error line [{0}]:{1}\nChange Direction : \n\n[1]\n{2}\n{3}\n{4}\n\n[2]\n{2}\n{5}\n{4}".format(errline+1,buffer[errline],buffer[errline-1],result1,buffer[errline+1], result2))
                    ...
                    try:
                        ...
                        if 'NameError: ' in ERR[-2]: # v2
                            ...
                            if input("\nDo you want to automatically change above line? [y/n]  ") == 'y':
                                ...
                                buffer[errline] = buffer[errline].replace(err_var[1:-1], list(self.var_dict.keys())[line])
                                self.codes = "\n".join(buffer)
                                tree = ast.parse(self.codes)
                                self.tree = tree
